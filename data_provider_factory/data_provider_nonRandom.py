...
class NonRandom(DataProvider):
    ...
    def _get_one_real_time_price_from_local_variable_hist(self, security, timeNow, tickType, freq='1 min'):
        ...
        if (str_security, freq) in self._simulatedByDailyBars:
            timeNow = int(dt_to_epoch(timeNow))
            hist = self._hist[str_security]['1 day']
            if timeNow in hist.index:
                ans = hist.loc[timeNow, self._useColumnNameWhenSimulatedByDailyBar]
                self._log.debug(__name__ + '::_get_one_real_time_price_from_local_variable_hist: simulatedByDailyBars str_security=%s timeNow=%s tickType=%s returnedValue=%s' % (str_security, timeNow, tickType, ans))
                return ans
            else:
                ...
                timeNowPosition = bisect.bisect_left(hist.index, timeNow) - 1
        ...
        if str_security not in self._hist:
            timeNow = dt.datetime.strftime(timeNow, "%Y%m%d %H:%M:%S %Z")  # datetime -> string
            if (security, timeNow) in self._1minHist:
                return self._1minHist[(security, timeNow)].iloc[-1][fieldName]
            if self.name in [DataProviderName.IB, DataProviderName.ROBINHOOD, DataProviderName.ROBINHOOD, DataProviderName.IBRIDGEPY]:
                self._log.info(__name__ + '::_get_one_real_time_price_from_local_variable_hist: Do not have hist for str_security=%s' % (str_security,))
                if security.secType != 'CASH':
                    return self._helper(security, timeNow, fieldName)
                else:
                    return self._helper(security, timeNow, fieldName, 'ASK')
            else:
                self._log.error(__name__ + '::_get_one_real_time_price_from_local_variable_hist: EXIT, not enough hist %s, %s from dataProviderName=%s' % (security, timeNow, self.name,))
                raise NotEnoughHist()
        ...
        if timeNow in hist.index:
            ...
            self._log.debug(__name__ + '::_get_one_real_time_price_from_local_variable_hist: str_security=%s timeNow=%s tickType=%s returnedValue=%s' % (str_security, timeNow, tickType, ans))
            return ans
        else:
            ...
            self._log.debug(
                __name__ + '::_get_one_real_time_price_from_local_variable_hist: str_security=%s timeNow=%s tickType=%s returnedValue=%s'
                % (str_security, timeNow, tickType, ans))
            return ans

    def _helper(self, security, timeNow, fieldName, priceType='TRADES'):
        self._log.info('IBridgePy has to request 1 min bar at %s from a broker %s to continue backtesting but it will be slow. Recommend to add HistIngestPlan in TEST_ME.py' % (timeNow, self.name,))
        hist = self.provide_hist_from_a_true_dataProvider(security, timeNow, '60 S', '1 min', priceType, useRTH=1, formatDate=1)
        self._1minHist[(security, timeNow)] = hist
